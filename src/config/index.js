...
const config = {
	...
	auth: { apiKey: process.env.API_KEY || "", jwtSecret: process.env.JWT_SECRET || "" },
	...
	providers: {
		...
		d7networks: { apiToken: process.env.D7_API_TOKEN || "", senderId: process.env.D7_SENDER_ID || "D7TEST" },
		sinch: {
		...
		textlocal: { apiKey: process.env.TEXTLOCAL_API_KEY || "", sender: process.env.TEXTLOCAL_SENDER || "TXTLCL" },
		gupshup: {
		...
		mtalkz: {
			...
			route: process.env.MTALKZ_ROUTE || "1",
			peid: process.env.MTALKZ_PEID || "",
			dltTemplateId: process.env.MTALKZ_DLT_TEMPLATE_ID || "",
		},
		clickatell: { apiKey: process.env.CLICKATELL_API_KEY || "", from: process.env.CLICKATELL_FROM || "" },
		brevo: { apiKey: process.env.BREVO_API_KEY || "", sender: process.env.BREVO_SENDER || "" },
		bulksms: {
			tokenId: process.env.BULKSMS_TOKEN_ID || "",
			tokenSecret: process.env.BULKSMS_TOKEN_SECRET || "",
			from: process.env.BULKSMS_FROM || "",
		},
		textmagic: {
			username: process.env.TEXTMAGIC_USERNAME || "",
			apiKey: process.env.TEXTMAGIC_API_KEY || "",
			senderId: process.env.TEXTMAGIC_SENDER_ID || "",
		},
		messagebird: {
		...
		netcore: {
			...
			senderId: process.env.NETCORE_SENDER_ID || "",
			peid: process.env.NETCORE_PEID || "",
			dltTemplateId: process.env.NETCORE_DLT_TEMPLATE_ID || "",
		},
		sarv: {
			apiKey: process.env.SARV_API_KEY || "",
			senderId: process.env.SARV_SENDER_ID || "",
			route: process.env.SARV_ROUTE || "TA",
			peid: process.env.SARV_PEID || "",
			dltTemplateId: process.env.SARV_DLT_TEMPLATE_ID || "",
		},
		pinnacle: {
			userId: process.env.PINNACLE_USER_ID || "",
			password: process.env.PINNACLE_PASSWORD || "",
			senderId: process.env.PINNACLE_SENDER_ID || "",
			transRoute: process.env.PINNACLE_TRANS_ROUTE || "01",
			promoRoute: process.env.PINNACLE_PROMO_ROUTE || "06",
			peid: process.env.PINNACLE_PEID || "",
			dltTemplateId: process.env.PINNACLE_DLT_TEMPLATE_ID || "",
		},
		unifonic: { appSid: process.env.UNIFONIC_APP_SID || "", senderId: process.env.UNIFONIC_SENDER_ID || "" },
	},
};
